...
(*----------------------------------------------------------------------------*)
begin
(* JAC! if buf_i > 0 then blockwrite(dst,t_buf,buf_i); *)
 buf_i := 0;
end;
...
begin
 ...
 if okej then begin
(* JAC!  writeln(mmm,#13#10,'; ',s); *)
  for i:=High(t_mad)-1 downto 0 do
   if t_mad[i].bnk=b then
(* JAC!    if t_mad[i].typ=tst then writeln(mmm,t_mad[i].nam,#9,'=',#9,'$',Hex(t_mad[i].adr,4)); *)
 end;
     ...
     if macro_rept_if_test then begin
       ...
       if txt<>'' then begin
         ...
         if pass=pass_end then begin
           ...
           (* JAC! blockwrite(g, t_get[idx_get], _doo); *)
           closefile(g);
         end;
